...
## What ships today: social sign-in

The account entry point currently offers **Continue with Google** and **Continue with Apple** through Auth0, not UCSD. UCSD federation is still planned; `Session.jsx` keeps `campusLogin` and `campusConfigured`, but no UI reaches them yet.

A social login proves control of a Google or Apple account and nothing else. It is not enrollment, not a UCSD address, and not authorization — shared marketplace access stays gated server-side exactly as before.

## What is proven by tests

| Link | Proven by | Command |
| --- | --- | --- |
| Google and Apple each send Auth0 their own connection, over PKCE, returning only to this origin | `tests/signin/social-signin.spec.js` | `npm run test:signin` |
| A missing Apple provider disables only Apple and explains itself | `tests/signin/apple-unconfigured.spec.js` | `npm run test:signin` |
| With no Auth0 application, sign-in is disabled and no provider is contacted | `tests/signin/signin-unconfigured.spec.js` | `npm run test:signin` |
| No password field exists in this origin | `tests/signin/social-signin.spec.js` | `npm run test:signin` |
| Signing in is never presented as proof of enrollment | `tests/signin/social-signin.spec.js` | `npm run test:signin` |
| *(UCSD, for later)* Action denies wrong connection, lookalike domains, unverified email; stamps the campus claim | `tests/campus-login.test.js` | `npm test` |
| *(UCSD, for later)* Server rejects forged/missing campus claims and fails closed with no connection set | `backend/test_scanner.py` | `python -m unittest backend.test_scanner` |

The sign-in run starts three throwaway dev servers, points the configured one at a reserved `.example` tenant, and aborts every request to it. No real credential or tenant is involved. Each server sets every Auth0 variable explicitly, including to empty, because Vite also loads the gitignored `.env`; relying on omission would make results depend on whose machine they run on.
...
2. **Blocked on the Auth0 tenant.** The SPA application already exists — `.env` supplies `VITE_AUTH0_DOMAIN`, `VITE_AUTH0_CLIENT_ID` and `VITE_AUTH0_AUDIENCE`, which is what social sign-in runs on. What is missing is the *enterprise connection*, which can only be created once UCSD supplies metadata. Check entitlement: enterprise connections are typically a paid tier.
3. **Then configuration only**, no code changes:
   - `VITE_UCSD_AUTH0_CONNECTION` set to the exact connection name (unset today, so `campusLogin` rejects). A UI entry point for it also has to come back, since the account card now offers Google and Apple.
